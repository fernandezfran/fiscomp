...
real(pr)             :: Ekinm, Epotm, Etotm, Tempm, Presm
...
do i = 1, teq
    ...
    if (mod(i,10) == 0) then !: write trajectory
        write(36,*) N; write(36,*)
        do j = 1, N; write(36,'(A,3(E15.6,2x))') 'Ar', x(j), y(j), z(j); enddo
    endif
...
!### main loop
Ekinm = 0._pr; Epotm = 0._pr; Etotm = 0._pr; Tempm = 0._pr; Presm = 0._pr
do i = teq + 1, teq + trun
    ...
    !acumulo variables para calcular valores medios
    Ekinm = Ekinm + Ekin
    Epotm = Epotm + Epot
    Etotm = Etotm + (Ekin + Epot)
    Tempm = Tempm + Temp
    Presm = Presm + Pres
    !
...
write(45,'(A,x,E15.6)') '# energía cinética media  : ', Ekinm/real(trun,pr) 
write(45,'(A,x,E15.6)') '# energía potencial media : ', Epotm/real(trun,pr) 
write(45,'(A,x,E15.6)') '# energía total media     : ', Etotm/real(trun,pr) 
write(45,'(A,x,E15.6)') '# temperatura media       : ', Tempm/real(trun,pr) 
write(45,'(A,x,E15.6)') '# presión media           : ', Presm/real(trun,pr) 
