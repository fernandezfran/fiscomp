...
!
!si quiero la distribución f(x) = x**(-2), para x >= 1, entonces,
!x tiene que tomar valores con la forma -1/u
!
! $ gfortran a-potencia.f90 *.o && ./a.out
!
...
open(15, file='a-histo.dat', status='replace')

nbin    = 25                        !cantidad de intervalos del histograma
allocate( hist(nbin) )
hist(:) = 0
...
function finv(u)
!función inversa para obtener distribución no uniform x**(-2)
implicit none
real(pr), intent(in) :: u
real(pr)             :: finv
