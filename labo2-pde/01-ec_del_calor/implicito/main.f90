...
! utilizando el método implicito
!
...
filename = 'temp_implicito.dat' 
frmt = '(5(E15.6,2x))'
open(55, file=filename, status='replace')

!main loop - implicito y exacto
...
do j = 1, 20000
    ...
    b_i(:) = T_antes(:)
    b_i(1) = eta*T_0 + T_antes(1)
    b_i(n) = eta*T_0 + T_antes(n)
...
if (timemes.eq.0) then
    ...
    enddo
    call cpu_time(time2)
    write(*,*) 'metodo implicito', time2 - time1
