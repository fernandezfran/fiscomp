...
use initialization, only : x, y, z, T, N, L, V, rho, rcut, initial_cond => xyzsc
...
real(pr)                 :: delta, xnew, ynew, znew, dU, utail, ptail, tasa
...
write(30,*) N; write(30,*)
do j = 1, N; write(30,'(A,3(E15.6,2x))') 'C', x(j), y(j), z(j); enddo
...
do i = 1, mceq
    ...
    do j = 1, N
        ...
        if (dU <= 0._pr .or. ranf() <= exp(-dU/T)) then
            ...
            x(part) = xnew - L*anint(xnew/L)
            y(part) = ynew - L*anint(ynew/L)
            z(part) = znew - L*anint(znew/L)
            accept  = accept + 1
        endif
    enddo
